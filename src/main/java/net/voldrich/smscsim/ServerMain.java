 ...
 *
 * <ul>
 *   <li>For each submit request it receives it sends a delivery receipt with configurable delay.
 *   <li>Supports listening on multiple ports and multiple applications.
 *   <li>Sending of segmented SMPP messages, supports segmentation set via optional parameter or via
 *       UDH00 or UDH08
 *   <li>Outgoing (MO) messages are sent to connected set of RX and TRX connections with the same
 *       application/system ID. RoundRobin is used to rotate between them.
 *   <li>Controlling MO messages sent by simulator with JMX commands - start, stop, send message
 *       stream and so on
 * </ul>
 *
  ...
  public ServerMain() {
    ...
    for (Integer port : PARAMS.getSmscPortsAsIntegers()) {
      SmppServerConfiguration serverConfig =
          context.getBean(SmppServerConfiguration.class); // new configuration instance every time
