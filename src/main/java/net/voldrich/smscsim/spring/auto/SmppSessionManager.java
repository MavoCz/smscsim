...
/**
 * Manages all RX and TRX bound SMPP connections represented by session instance. These connections
 * are kept in global list and separate list for each distinct application id. Class is thread safe.
 * When SMSCsim wants to send out a MO message, it requests a session by calling
 * getNextServerSession. A session is chosen based on round robin algorithm.
 */
@Component
public class SmppSessionManager {
  ...
  /**
   * Internal session list implementing simple round robin using a cycle iterator. This class is not
   * thread safe.
   */
